...
export function GoogleAIResults({ results }: { results: SearchResult }) {
  ...
  if (results.searchCount === 0) {
    return <p className="text-gray-600">No AI Overview results found for this keyword.</p>
  }

  if (results.foundCount === 0) {
    return <p className="text-gray-600">AI Overview found, but your brand was not mentioned in any results.</p>
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6 text-center">
        ...
        <Card className="p-4">
          <p className="text-sm text-gray-500 uppercase">Found In Searches</p>
        ...
        <Card className="p-4">
          <p className="text-sm text-gray-500 uppercase">Total Searches</p>
      ...
      <div className="text-sm text-gray-500 mb-3">
        <p>
          Your brand was found in {results.foundCount} out of {results.searchCount} AI Overview results.
        </p>
      </div>
          ...
          <ul className="space-y-4">
            {results.searches.map((search, index) => (
              <li key={index} className="border-b pb-3 last:border-0">
                ...
                {search.searchDetails?.contentSnippet && (
                  <div className="mt-1 text-xs text-gray-500 italic">"{search.searchDetails.contentSnippet}"</div>
                )}
                {search.sentiment && (
                  <div className="mt-1">
                    <span
                ...
                {showDebug && search.searchDetails?.debugInfo && (
                  <div className="mt-1 text-xs bg-gray-200 p-2 rounded">
                    <strong>Debug:</strong> {search.searchDetails.debugInfo}
                  </div>
                )}
              </li>
            ))}
